...
{
    public class AverageTicketPriceController : ApiController
    {
        ...
        public AveragePrice Get(string originCountry, string originCity, int duration)
        {
            ...
            foreach (AllMonthTicketsFromCity c in cursor)
            {
                ...
                {
                    if (!chp.city.Equals(c.city) && chp.city.Equals(price.info[counter].city))
                    {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                if (list[i].city.Equals(originCity))
                {
                    flag = i;
                    break;
                }
